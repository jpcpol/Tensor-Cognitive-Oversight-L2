...
\begin{table}[t]
  ...
  \begin{tabular}{clcc}
    \toprule
    \# & Dimension & Source & Inverted \\
    \midrule
    $v_1$ & functional\_correctness & LLM-QA & --- \\
    $v_2$ & architectural\_alignment & LLM-QA & --- \\
    $v_3$ & scalability\_projection & LLM-QA & --- \\
    $v_4$ & security\_risk & Bandit & $\checkmark$ \\
    $v_5$ & observability\_coverage & Radon & --- \\
    $v_6$ & testability & Radon & $\checkmark$ CC \\
    $v_7$ & maintainability & Radon & $\checkmark$ Halstead \\
    $v_8$ & technical\_debt & Radon & $\checkmark$ MI \\
    $v_9$ & performance & LLM-QA & --- \\
...
\begin{itemize}
  ...
  \item \textbf{LLM-QA} (semantic): structured tool-use evaluation of $v_1, v_2, v_3, v_9$
    using few-shot prompting with three calibration examples per artifact type.
\end{itemize}
